											...
											INT8U           *perr)
{
	...
	//  新增消息驱动字段初始化
	ptmr->OSTmrMsgDriven   = 0;  // 默认使用回调函数
  ptmr->OSTmrTaskID      = 0;
  ptmr->OSTmrMsgCode     = 0;
  ptmr->OSTmrSendInst    = 0;
  ptmr->OSTmrRecvInst    = 0;
...
/*
*********************************************************************************************************
*                        CREATE A MESSAGE-DRIVEN TIMER
*
* Description: This function is called by your application code to create a message-driven timer.
*
* Arguments  : dly           Initial delay.
*              period        The 'period' being repeated for timer.
*              opt           Specifies either: OS_TMR_OPT_ONE_SHOT or OS_TMR_OPT_PERIODIC
*              task_id       Target task ID to receive timer message.
*              msg_code      Message code to send when timer expires.
*              send_inst     Sender instance ID.
*              recv_inst     Receiver instance ID.
*              pname         Pointer to timer name string.
...
*/
#if OS_TMR_EN > 0u
OS_TMR *OSTmrCreateMsgDriven (INT32U           dly,
                               INT32U           period,
                               INT8U            opt,
                               INT16S           task_id,
                               INT16U           msg_code,
                               ...
                               INT8U           *perr)
{
    ...
    if (task_id < 0) {  // Valid task ID should be >= 0
    ...
    ptmr->OSTmrPeriod      = period;
    ptmr->OSTmrOpt         = opt;
    ptmr->OSTmrCallback    = (OS_TMR_CALLBACK)0;  // 不使用回调
    ptmr->OSTmrCallbackArg = (void *)0;
    
    // 设置消息驱动字段
    ptmr->OSTmrMsgDriven   = 1;  // 使用消息驱动
    ptmr->OSTmrTaskID      = task_id;
    ptmr->OSTmrMsgCode     = msg_code;
    ptmr->OSTmrSendInst    = send_inst;
    ptmr->OSTmrRecvInst    = recv_inst;
	...
	for (;;)
	{
		...
		while (ptmr != (OS_TMR *)0)
		{
			...
			if (OSTmrTime == ptmr->OSTmrMatch)                   /* Process each timer that expires                   */
			{
				...
				if	(ptmr->OSTmrMsgDriven == 1)
				{
					err = Msg_SendTimerExpired(ptmr, 
                                             ptmr->OSTmrTaskID, 
                                             ptmr->OSTmrMsgCode,
                                             ptmr->OSTmrSendInst, 
                                             ptmr->OSTmrRecvInst);
